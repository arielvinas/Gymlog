...
| 2 | **`StreakCalculator.currentWeekStreak` agrupa por `weekTitle` (String), no por semana calendario.** ✅ **CONFIRMADO** (U-32): dos semanas con el mismo título → racha **1 en vez de 2**; una semana partida en dos títulos → **2 en vez de 1**. El error va para los dos lados. **Alcanzable**, pero no de inmediato: `WeekAssigner` titula `"Semana del 15 jun"` **sin año** (colisiona recién en 2037), y el plan sembrado usa **títulos fijos** `"Semana 1".."Semana 5"` — un bloque nuevo que renumere desde "Semana 1" fusiona su primera semana con la vieja. Raíz: **un texto para mostrar se usa como identidad**. Fix: agrupar por inicio de semana calendario, como ya hace `WeeklyVolume`. **Pendiente.** | U-32 ✅ |
...
- [x] **U-32** ⚠️ **Bug 2 — CONFIRMADO.** ✅ Ver la tabla de bugs. Los dos modos de falla están
